...
cluster_name="splash2-emu"
cluster_desc="
Run emulation of the SPLASH-2 benchmark suite. For each benchmark, the number of
threads is set to powers of 2 between 1 and 16.
	...
	do
		...
		do
			$m2s_cluster_sh add $cluster_name $bench/$nthreads \
				-p $nthreads splash2/$bench || exit 1
		done
	done
